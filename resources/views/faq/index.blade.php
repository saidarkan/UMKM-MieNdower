                        ...
                        @forelse ($faqs as $faq)
                            <tr class="border-t">
                                ...
                                <td class="px-4 py-2 text-sm text-gray-800">
                                    <a href="{{ route('faq.edit', $faq->id) }}" class="bg-blue-500 hover:bg-blue-600 text-white rounded-full py-2 px-4 transition duration-300 ease-in-out transform hover:scale-105">
                                        <i class="fas fa-edit"></i> Edit
                                    </a>
                                    |
                                    <form action="{{ route('faq.destroy', $faq->id) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus menu ini?');">
                                        @csrf
                                        @method('DELETE')
